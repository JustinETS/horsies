...
io.on('connection', (socket) => {
  ...
  socket.on('create lobby', () => {
    console.log('Tracking new lobby');
    isLobbyActive = true;
    console.log(`create lobby - ${socket.user_id}`);
  ...
  socket.on('join lobby', () => {
    console.log(`Player joined lobby - ${socket.user_id}`);
    socket.join('lobby');
    socket.emit('lobby joined');
  });

  socket.on('leave lobby', () => {
    console.log(`Player left lobby - ${socket.user_id}`);
    socket.leave('lobby');
  });

  socket.on('player ready', () => {
    console.log(`Player ready - ${socket.user_id}`);
    socket.to('lobby').emit('player ready update');
  });

  socket.on('start game', () => {
    console.log(`Game starting - ${socket.user_id}`);
    io.to('lobby').emit('game start');
  });

  socket.on('button pressed', (buttonText) => {
    console.log(`Button pressed: ${buttonText} - ${socket.user_id}`);
    io.emit('display text', buttonText);
  });

  socket.on('horse names', (names) => {
    console.log('Horse names received:', names);
    console.log(`CurrentSession - ${socket.user_id}`);
